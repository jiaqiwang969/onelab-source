function  [A1n,A2n, B1n,B2n] = cylindrical_wall_s_out_abc2_Pade(L,theta,n,kp,ks,eps_p,eps_s,a,b,lambda,mu,rho,omega)
 ...
 bessel_family(n,kp,ks,a,b);
  ...
  [Aj,Bj] = ComplexPadeISqrt2(L,theta);
  zp=-(n/(kpeps*b))^2;
  zs=-(n/(kseps*b))^2;
  xip = 0;
  xis = 0;
  for ind=0:L-1
    xip = xip + Aj(ind+1)./(Bj(ind+1)+zp); 
    xis = xis + Aj(ind+1)./(Bj(ind+1)+zs); 
  end
  xip=xip/kp;
  xis=xis/ks;

  det = 1.d0 + ((n/b)**2) * xip * xis; 
  ...
  M(4,4) = -mu*dr2_h2_s + (D2-mu)*(-(n/b)**2)*besselh(n,2,ks*b) - D4*dr_h2_s;
    ...
    %    disp(['n:', num2str(n),'M:', num2str(M(1,1))])
    
  F(1) = psi_0*epsilon*((-1i)**n)*(n/a)*besselj(n,ks*a);
  F(2) = psi_0*epsilon*((-1i)**n)*dr_j_s;
  F(3) = 0;
  F(4) = 0;
